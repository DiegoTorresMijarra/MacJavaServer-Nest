...
describe('RestaurantesController', () => {
  ...
  describe('findAll', () => {
    it('debe devolver array de todos los restaurantes ', async () => {
      const mockRestaurants = [{ name: 'Restaurant A' }, { name: 'Restaurant B' }];
    ...
    it('debe mostrar un log con la accion', async () => {
      ...
      expect(loggerSpy).toHaveBeenCalledWith('Pidiendo todos los restaurantes (Controller)');
    });
  });
  ...
  describe('findAllPaginated', () => {
    it('debe mostrar los restaurantes paginados', async () => {
      ...
      const mockPaginatedRestaurants = [{ name: 'Restaurante 1' }, { name: 'Restaurante 2' }];
      mockRestaurantesService.findAllPaginated.mockResolvedValue(mockPaginatedRestaurants);
  ...
  describe('create', () => {
    it('debe crear un restaurante', async () => {
      const dto : CreateRestauranteDto = {nombre: 'Restaurante test', calle: 'Calle test', localidad: 'Localidad test', capacidad: 100, borrado: false};
      const mockRestaurant : Restaurante = new Restaurante();
    ...
    it('debe mostrar un error BadRequest si se ingresan datos mal', () => {
        const dto : CreateRestauranteDto = {nombre: '', calle: 'Calle test', localidad: 'Localidad test', capacidad: 100, borrado: false};
  ...
  describe('findByName', () => {
    ...
    it('debe lanzar un erro NotFoundException si no existe el Restaurante', async () => {
      const nombre='Restaurante test';
        jest.spyOn(service, 'findByName').mockRejectedValue(new NotFoundException());
        await expect(controller.findByName(nombre)).rejects.toThrow(NotFoundException);
    });
  });
});
